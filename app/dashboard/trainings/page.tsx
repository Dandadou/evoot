...
export default function TrainingsPage(){return <main className="editorShell"><header className="editorTop"><div><Link className="brand" href="/">ÉV<span>OO</span>T!</Link><span className="crumb"> / Évolution PME / Formations</span></div><Link className="secondary" href="/dashboard">Tableau de bord</Link></header><section className="editorHero"><div className="eyebrow">PORTAIL ENTREPRISE · ÉVOLUTION PME</div><h1>Construire les formations.</h1><p>Choisis un bloc pour préparer ses questions live, ses relances formateur et sa synthèse pédagogique.</p><button className="primary">+ Nouvelle formation</button></section><section className="catalogue">{blocks.map((b,i)=><Link href={i===0?'/dashboard/trainings/comprendre-ses-reactions':'#'} className="blockCard" key={b[0]}><div className="blockIndex">{String(i+1).padStart(2,'0')}</div><div><small>BLOC ÉVOLUTION PME</small><h2>{b[0]}</h2><p>{b[1]}</p></div><div className="blockAction">{i===0?'Ouvrir l’éditeur →':'Préparer →'}</div></Link>)}</section></main>}
