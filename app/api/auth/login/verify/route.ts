...
export async function POST(req:Request){
 try{
  ...
  const isEvootAdmin=active.some(m=>m.organizationId==='evoot'&&m.role==='EVOOT_ADMIN');const customer=active.find(m=>m.role!=='EVOOT_ADMIN');const sessionToken=randomBytes(32).toString('base64url');const expires=new Date(Date.now()+SESSION_DAYS*86400000);
  await db.prepare(`INSERT INTO auth_sessions (user_id,token_hash,active_organization_id,expires_at) VALUES (?,?,?,?)`).bind(user.id,hash(sessionToken),customer?.organizationId??null,expires.toISOString()).run();
