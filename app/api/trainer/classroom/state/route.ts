...
export async function PATCH(req:Request){try{const auth=await requireOrganizationRole('TRAINER');const db=getDB(),org=auth.organizationId!,b=await req.json(),sessionId=Number(b.sessionId);if(!sessionId)return NextResponse.json({error:'Séance requise'},{status:400});const s=await sessionForTrainer(db,org,auth.userId,auth.roles.includes('ORG_ADMIN'),sessionId);if(!s)return NextResponse.json({error:'Séance introuvable'},{status:404});
 ...
 else if(b.action==='projection'){await db.prepare(`INSERT INTO course_session_state(course_session_id,organization_id,projection_enabled,updated_by_user_id,updated_at) VALUES(?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(course_session_id) DO UPDATE SET projection_enabled=excluded.projection_enabled,updated_by_user_id=excluded.updated_by_user_id,updated_at=CURRENT_TIMESTAMP`).bind(sessionId,org,b.enabled?1:0,auth.userId).run();}
 else if(b.action==='interaction'){const interactionId=Number(b.interactionId);const r=await db.prepare(`UPDATE classroom_interactions SET status='RESOLVED',resolved_at=CURRENT_TIMESTAMP WHERE id=? AND organization_id=? AND course_session_id=? AND status='OPEN'`).bind(interactionId,org,sessionId).run();if(!r.meta?.changes)return NextResponse.json({error:'Interaction introuvable'},{status:404});}
 else if(b.action==='status'){const status=String(b.status||'');if(!['SCHEDULED','IN_PROGRESS','COMPLETED','CANCELLED'].includes(status))return NextResponse.json({error:'Statut invalide'},{status:400});await db.prepare(`UPDATE course_sessions SET status=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND organization_id=?`).bind(status,sessionId,org).run();}
 else return NextResponse.json({error:'Action invalide'},{status:400});return NextResponse.json({ok:true});}catch(e){if(e instanceof Error&&e.message==='UNAUTHORIZED')return NextResponse.json({error:'Non autorisé'},{status:401});console.error(e);return NextResponse.json({error:'Commande impossible'},{status:500})}}
