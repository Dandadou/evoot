...
export async function PUT(request:Request, context:{params:Promise<{slug:string}>}){
 ...
 if(!training)return NextResponse.json({error:'Formation introuvable.'},{status:404});
 const target=body.targetDurationMinutes===null||body.targetDurationMinutes===''?null:Number.isFinite(Number(body.targetDurationMinutes))?Math.max(0,Number(body.targetDurationMinutes)):null;
